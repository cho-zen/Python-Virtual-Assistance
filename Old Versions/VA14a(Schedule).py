...
class Message():
    ...
    def showtodo(self):
        return ['show to do','show my to do list','what about to do list',' show my work list','show work list']
...
class Speak():
    ...
    def timingInitMessage(self):
        time = (datetime.datetime.now().hour)
        if time >= 0 and time < 12:
            return f'''It's {datetime.datetime.now().strftime("%H %M")}..O'clock..of..{datetime.datetime.now().strftime("%A")}...\n'''
        elif time>=12 and time <16:
            return f'''It's {datetime.datetime.now().strftime("%H %M")}..O'clock..of..{datetime.datetime.now().strftime("%A")}...\n'''
        elif time >=16 and time <=20:
            return f'''It's {datetime.datetime.now().strftime("%H %M")}..O'clock..of..{datetime.datetime.now().strftime("%A")}...\n'''
    ...
    def weatherForcasting(self):
        ...
        if clouds<=30:
            ...
            msg = ""
        return f'''Today weather outside will be, {description}.\nWith the tempeture of {temp} degree celsius, but is feels like {feelslike} degree celsius \nClouds appear in the sky, with the percentage of {clouds} \n {msg}'''
    ...
    def weather(self):
        ...
        return f'''Today weather outside will be, {description}....\nWith the tempeture of, {temp}....degree celsius'''


    def time(self):
        time = datetime.datetime.now().hour
        if time<=12:
            return f'''Time right now is {datetime.datetime.now().strftime("%H, %M")} AM \nHope your are up to your schedule'''
        else:
    ...
    while True:
        ...
        if len(userinput)>0:
            ...
            for i in message.wishMessage():
                if i in inputlist:
                    say = speak.wishMessage(i,fun = speak.timingInitMessage(),fun2 = speak.wishmeright(), fun3 = speak.weather())
                    print(say)
                    engine.say(say)
                    engine.runAndWait()
                    break
            ...
            for i in message.changeVoiceto():
                if i in inputlist:
                    ...
                    engine.say(saycvt)
                    engine.runAndWait()      
            for i in message.scheduleshow():
                if i in inputlist:
                    print(speak.scheduleshow())
                    engine.say(speak.scheduleshow()) 
                    engine.runAndWait()
                    break   
